...
/**
 * Initial admin setup page — shown only when no users exist in the system.
 *
 * Creates the first admin account and redirects to the dashboard.
 */
import { ref } from 'vue'
...
/** True while the setup request is in flight. */
const loading = ref(false)

/** Error message shown when setup fails. */
const error = ref<string | null>(null)

/**
 * Submits the setup form to create the initial admin account.
 *
 * @returns Promise that resolves after setup attempt.
 */
async function handleSetup(): Promise<void> {
  ...
  loading.value = true
  try {
    const data = await request<{ accessToken: string; role: string }>('/auth/setup', {
      method: 'POST',
      body: JSON.stringify({
        email: email.value,
...
<template>
  <div class="relative min-h-screen bg-surface-base flex items-center justify-center overflow-hidden">
    ...
    >
      ...
      <!-- Heading -->
      <div class="mb-7">
        <h1 class="font-display text-[1.6rem] font-bold text-text-primary tracking-tight leading-none mb-1.5">Initial Setup</h1>
        <p class="text-sm text-text-secondary">Create your admin account to get started</p>
      </div>
      ...
      <form @submit.prevent="handleSetup" class="flex flex-col gap-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="flex flex-col gap-1.5">
            <label class="text-[0.72rem] font-semibold uppercase tracking-[0.08em] text-text-muted">First name</label>
            <input
              v-model="firstName"
              type="text"
              required
              placeholder="John"
              class="w-full bg-white/[0.04] border border-white/[0.08] rounded-[10px] px-3 py-2.5 text-sm text-text-primary placeholder:text-text-muted outline-none transition-all duration-200 focus:border-primary-500/50 focus:bg-primary-500/[0.04] focus:ring-2 focus:ring-primary-500/10"
            />
          </div>
          <div class="flex flex-col gap-1.5">
            <label class="text-[0.72rem] font-semibold uppercase tracking-[0.08em] text-text-muted">Last name</label>
            <input
              v-model="lastName"
              type="text"
              required
        ...
        <!-- Confirm password -->
        <div class="flex flex-col gap-1.5">
          <label class="text-[0.72rem] font-semibold uppercase tracking-[0.08em] text-text-muted">Confirm password</label>
          <div class="relative">
            <svg class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
              <rect x="3" y="11" width="18" height="11" rx="2" ry="2"/>
              <path d="M7 11V7a5 5 0 0 1 10 0v4"/>
