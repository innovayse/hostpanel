...
/**
 * Invoice notes tab — admin notes textarea with Save Changes button.
...
const props = defineProps<{
  ...
  invoice: Invoice
  /** When true, notes are read-only and Invoice Items table is read-only. */
  readonly?: boolean
}>()

const emit = defineEmits<{
  /** Emitted after notes or items are saved. */
  updated: []
}>()
...
async function handleSaveNotes(): Promise<void> {
  ...
  await store.updateNotes(props.invoice.id, notes.value)
...
<template>
  <div>
    <!-- View mode: warning if no notes, otherwise read-only display -->
    <template v-if="readonly">
      <AppAlert v-if="!invoice.notes" variant="warning" class="mb-5">
        There are no notes on this invoice.
      </AppAlert>
      <div v-else class="bg-surface-card border border-border rounded-2xl p-5 mb-5">
        <p class="text-[0.82rem] text-text-primary whitespace-pre-wrap">{{ invoice.notes }}</p>
      </div>
    </template>
    ...
    <template v-else>
      <div class="bg-surface-card border border-border rounded-2xl p-5 mb-5">
        <textarea
          ...
          class="w-full min-h-[10rem] bg-white/[0.04] border border-border rounded-[10px] px-3 py-2 text-[0.82rem] text-text-primary placeholder-text-muted focus:outline-none focus:border-primary-500/50 focus:ring-1 focus:ring-primary-500/10 transition-colors resize-y mb-4"
        />
