...
const filteredTransactions = computed(() => {
  ...
  return transactions.filter(tx => {
    if (appliedFilters.value.show !== 'all') {
      if (appliedFilters.value.show === 'in' && tx.type !== 'Credit') return false
      if (appliedFilters.value.show === 'out' && tx.type !== 'Debit') return false
    }

    if (appliedFilters.value.paymentMethod && tx.gateway !== appliedFilters.value.paymentMethod) return false
    ...
    if (appliedFilters.value.amount > 0 && tx.amount !== appliedFilters.value.amount) return false

    if (appliedFilters.value.transactionId && !tx.id.toString().includes(appliedFilters.value.transactionId)) return false
  ...
  if (appliedFilters.value.dateRange) {
    ...
    allTransactions.forEach(tx => {
      ...
      if (txDate >= prevStartStr && txDate <= prevEndStr) {
        // Check other filters
        if (appliedFilters.value.show !== 'all') {
          if (appliedFilters.value.show === 'in' && tx.type !== 'Credit') return
          if (appliedFilters.value.show === 'out' && tx.type !== 'Debit') return
        }
        if (appliedFilters.value.paymentMethod && tx.gateway !== appliedFilters.value.paymentMethod) return
        ...
        if (appliedFilters.value.amount > 0 && tx.amount !== appliedFilters.value.amount) return
        if (appliedFilters.value.transactionId && !tx.id.toString().includes(appliedFilters.value.transactionId)) return

        if (tx.type === 'Credit') {
          prevTotalIn += tx.amount
        } else {
          prevTotalOut += tx.amount
        }
        prevTotalFees += tx.fees
      }
    })
  } else {
      ...
      <div class="bg-surface-card border border-border rounded-2xl overflow-hidden">
        ...
        <template v-if="filteredTransactions.length > 0">
          ...
          >
            ...
            <!-- Date -->
            <span class="text-[0.82rem] text-text-muted">{{ new Date(tx.createdAt).toLocaleDateString() }}</span>

            <!-- Payment Method -->
            <span class="text-[0.82rem] text-text-primary">{{ tx.gateway || 'Manual' }}</span>

            <!-- Description -->
            <span class="text-[0.82rem] text-text-secondary truncate">{{ tx.description }}</span>

            <!-- Amount In -->
            <span class="text-[0.82rem] font-medium" :class="tx.type === 'Credit' ? 'text-status-green' : 'text-text-muted'">
              {{ tx.type === 'Credit' ? `${tx.amount.toFixed(2)} ${tx.currency}` : '—' }}
            </span>

            <!-- Fees -->
            <span class="text-[0.82rem] font-medium text-text-primary">{{ tx.fees.toFixed(2) }} {{ tx.currency }}</span>

            <!-- Amount Out -->
            <span class="text-[0.82rem] font-medium" :class="tx.type === 'Debit' ? 'text-status-red' : 'text-text-muted'">
              {{ tx.type === 'Debit' ? `${tx.amount.toFixed(2)} ${tx.currency}` : '—' }}
            </span>
