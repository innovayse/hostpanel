...
/**
 * Live status sidebar for the CWP7 integration config page.
 *
 * Displays connection badge, packages count, and last tested time.
 * Emits `test` when the user clicks Test Connection.
 */
import { computed } from 'vue'
import type { Cwp7ServerInfoDto } from '../types/cwp7.types'

/** Props for Cwp7StatusSidebar. */
const props = defineProps<{
  /** CWP7 server info data, null while loading or on error. */
  info: Cwp7ServerInfoDto | null
...
<template>
  <div class="bg-surface-card border border-border rounded-2xl p-5 flex flex-col gap-5">
    ...
    <div v-if="props.info?.connected">
      <p class="text-[0.68rem] font-semibold uppercase tracking-[0.08em] text-text-muted mb-1">Packages</p>
      <p class="font-display text-3xl font-bold text-text-primary">{{ props.info.packagesCount ?? '—' }}</p>
    </div>
