namespace Innovayse.Application.Domains.Commands.AddMyDomainEmailForwardingRule;

/// <summary>Command for a client to add an email forwarding rule to one of their own domains.</summary>
/// <remarks>
/// Carries a domain id but no client id. Which account the domain must belong to is resolved inside
/// the handler from the credential, so the scoping cannot be separated from the message the way an
/// id in the body can. The admin route that may act on any client's domain dispatches
/// <c>AddEmailForwardingRuleCommand</c> directly.
/// </remarks>
/// <param name="DomainId">Primary key of the domain, which must belong to the caller.</param>
/// <param name="Source">Source alias or local part (e.g. "info").</param>
/// <param name="Destination">Destination email address.</param>
public sealed record AddMyDomainEmailForwardingRuleCommand(
    int DomainId,
    string Source,
    string Destination);
