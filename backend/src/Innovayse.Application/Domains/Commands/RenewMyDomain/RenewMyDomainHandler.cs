...
/// <summary>
/// Renews one of the calling client's own domains, refusing every domain that is not theirs.
/// </summary>
/// <remarks>
/// The check lives here rather than at the endpoint, so it travels with the message: nothing can
/// spend money renewing a registration through <see cref="RenewMyDomainCommand"/> without it having
/// run. Once ownership is settled the work is the same the admin route performs, so this dispatches
/// <see cref="RenewDomainCommand"/> rather than duplicating it.
/// </remarks>
...
public sealed class RenewMyDomainHandler(IDomainOwnership ownership, IMessageBus bus)
{
    ...
    /// <returns>A task that completes once the renewal has been placed.</returns>
    ...
    public async Task HandleAsync(RenewMyDomainCommand cmd, CancellationToken ct)
    {
        await ownership.RequireOwnedByCallerAsync(cmd.DomainId, ct);
        await bus.InvokeAsync(new RenewDomainCommand(cmd.DomainId, cmd.Years), ct);
    }
}
