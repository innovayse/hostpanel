...
/// <summary>
/// Sets automatic renewal on one of the calling client's own domains, refusing every domain that is
/// not theirs.
/// </summary>
/// <remarks>
/// The check lives here rather than at the endpoint, so it travels with the message: nothing can
/// change a renewal setting through <see cref="SetMyDomainAutoRenewCommand"/> without it having
/// run. Once ownership is settled the work is the same the admin route performs, so this dispatches
/// <see cref="SetAutoRenewCommand"/> rather than duplicating it.
/// </remarks>
...
public sealed class SetMyDomainAutoRenewHandler(IDomainOwnership ownership, IMessageBus bus)
{
    /// <summary>Handles <see cref="SetMyDomainAutoRenewCommand"/>.</summary>
    ...
    public async Task HandleAsync(SetMyDomainAutoRenewCommand cmd, CancellationToken ct)
    {
        await ownership.RequireOwnedByCallerAsync(cmd.DomainId, ct);
        await bus.InvokeAsync(new SetAutoRenewCommand(cmd.DomainId, cmd.Value), ct);
    }
}
