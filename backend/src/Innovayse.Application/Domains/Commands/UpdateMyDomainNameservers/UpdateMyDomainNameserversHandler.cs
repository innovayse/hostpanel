...
/// <summary>
/// Replaces the nameservers on one of the calling client's own domains, refusing every domain that
/// is not theirs.
/// </summary>
/// <remarks>
/// The check lives here rather than at the endpoint, so it travels with the message: nothing can
/// repoint a domain's nameservers through <see cref="UpdateMyDomainNameserversCommand"/> without it
/// having run. Once ownership is settled the work is the same the admin route performs, so this
/// dispatches <see cref="UpdateNameserversCommand"/> rather than duplicating it.
/// </remarks>
...
public sealed class UpdateMyDomainNameserversHandler(IDomainOwnership ownership, IMessageBus bus)
{
    /// <summary>Handles <see cref="UpdateMyDomainNameserversCommand"/>.</summary>
    /// <param name="cmd">The command. It names no account: this acts on the caller's own domain.</param>
    /// <param name="ct">Cancellation token.</param>
    /// <returns>A task that completes once the nameservers have been replaced.</returns>
    ...
    public async Task HandleAsync(UpdateMyDomainNameserversCommand cmd, CancellationToken ct)
    {
        await ownership.RequireOwnedByCallerAsync(cmd.DomainId, ct);
        await bus.InvokeAsync(new UpdateNameserversCommand(cmd.DomainId, cmd.Nameservers), ct);
    }
}
