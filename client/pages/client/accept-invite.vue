      ...
      <div class="relative p-8 rounded-2xl bg-gradient-to-br from-white/5 to-white/[0.02] border border-white/10 backdrop-blur-sm">
        ...
        <UiForm v-else :error="error" spacing="lg" @submit="handleSubmit">
          <UiInput
            v-model="password"
            type="password"
            :label="$t('client.acceptInvite.password')"
            :placeholder="$t('client.acceptInvite.passwordPlaceholder')"
          ...
          <UiInput
            ...
            type="password"
            :label="$t('client.acceptInvite.confirmPassword')"
            :placeholder="$t('client.acceptInvite.confirmPasswordPlaceholder')"
            autocomplete="new-password"
            required
          />
          <UiButton
...
/**
 * Accept invitation page — allows invited users to set their password
 * and create their account linked to a client.
 *
 * Reads the invitation token from the URL query parameter and presents
...
/** True when no valid token is present in the URL. */
...
/** Error message to display, empty when no error. */
const error = ref('')

/**
 * Validates the password inputs and submits the accept-invite request.
 * On success, redirects to the client dashboard with a full page reload
 * so middleware picks up the new auth cookies.
 */
async function handleSubmit(): Promise<void> {
  if (password.value !== confirmPassword.value) {
    error.value = t('client.acceptInvite.errorMismatch')
    return
  }

  if (password.value.length < 8) {
    error.value = t('client.acceptInvite.errorMinLength')
    return
  }
  ...
  try {
    ...
    error.value = fetchErr?.data?.statusMessage || t('client.acceptInvite.errorDefault')
  } finally {
    loading.value = false
  }
}
