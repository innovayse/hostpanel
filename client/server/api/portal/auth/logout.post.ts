 ...
 * 2. Fires backchannel logout to all platform services via app.innovayse.local
 *    (app.innovayse.local already knows all internal service URLs and keys).
...
export default defineEventHandler(async (event) => {
  ...
  // 2. Fire backchannel logout to all platform services via app.innovayse.local.
  ...
  if (authToken) {
    try {
      ...
      if (email) {
        fetch(`${mainApiUrl}/api/portal/auth/sso/logout-redirect?redirect_uri=${encodeURIComponent('http://panel.innovayse.local')}`, {
