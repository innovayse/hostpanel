          ...
          <aside class="lg:col-span-5 sticky top-24 space-y-6">
            <div class="p-8 bg-gradient-to-br from-[#1a1a24] to-[#121217] border border-white/10 rounded-[32px] shadow-2xl relative overflow-hidden ring-1 ring-white/10">
              ...
              <div
                ...
                class="mb-4 p-3 rounded-xl bg-yellow-500/10 border border-yellow-500/20 text-xs text-yellow-300"
              >
                {{ $t('cart.staleCurrencyNotice') }}
              </div>

              <div class="space-y-4 mb-8">
                <div v-for="item in cart.itemsInCurrency(currencyStore.payerCode)" :key="item.pid"
                  ...
                  <div class="text-sm font-black text-white whitespace-nowrap">{{ itemPrice(item) }}</div>
                </div>
              </div>
              ...
              <!-- Submit Button. Disabled with no method selected: the gateway list came back
                   empty because every gateway is switched off in the admin. -->
